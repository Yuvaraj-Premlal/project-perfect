...
async function fetchTemplates() {
  const r = await api.get('/api/ppap/templates')
  return r.data
}
async function fetchTemplate(id: string) {
  const r = await api.get(`/api/ppap/templates/${id}`)
  return r.data
}
interface PPAPElement { element_name: string; planned_days: number }

export default function PPAPTemplatesPage() {
  const qc = useQueryClient()
  const user = getCurrentUser()
  const canManage = user?.role === 'portfolio_manager' || user?.role === 'super_user'
  const { data: templates = [], isLoading } = useQuery({ queryKey: ['ppap-templates'], queryFn: fetchTemplates })
  const [showCreate, setShowCreate] = useState(false)
  const [viewId, setViewId] = useState<string | null>(null)
  const [deleting, setDeleting] = useState<string | null>(null)

  async function handleDelete(id: string, name: string) {
    if (!confirm(`Delete PPAP template "${name}"? This cannot be undone.`)) return
    setDeleting(id)
    try {
      await api.delete(`/api/ppap/templates/${id}`)
      qc.invalidateQueries({ queryKey: ['ppap-templates'] })
  ...
  return (
    <div>
      <div className="page-header" style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
        <div>
          <div className="page-title">PPAP Templates</div>
          <div className="page-sub">Define reusable PPAP element sets for your projects</div>
        </div>
        {canManage && <button className="tb-btn primary" onClick={() => setShowCreate(true)}>+ New Template</button>}
      </div>

      {templates.length === 0 ? (
        <div className="card" style={{ textAlign:'center', padding:56 }}>
          <div style={{ fontSize:15, fontWeight:600, color:'var(--text2)', marginBottom:6 }}>No PPAP templates yet</div>
          <div style={{ fontSize:13, color:'var(--text4)', marginBottom: canManage ? 24 : 0 }}>
            Create a template with PPAP elements and planned durations.
          </div>
      ...
      {showCreate && canManage && (
        <CreatePPAPTemplateModal
          onClose={() => setShowCreate(false)}
          onCreated={() => { setShowCreate(false); qc.invalidateQueries({ queryKey: ['ppap-templates'] }) }}
        />
      )}
      {viewId && <ViewPPAPTemplateModal templateId={viewId} onClose={() => setViewId(null)} />}
    </div>
  )
}

function CreatePPAPTemplateModal({ onClose, onCreated }: { onClose: () => void, onCreated: () => void }) {
  const [name, setName]         = useState('')
  const [description, setDesc]  = useState('')
  const [elements, setElements] = useState<PPAPElement[]>([{ element_name: '', planned_days: 10 }])
  ...
  async function handleSave() {
    ...
    try {
      await api.post('/api/ppap/templates', { name, description, elements })
      onCreated()
    } catch (err: any) {
    ...
    <div className="modal-overlay open">
      <div className="modal" style={{ maxWidth:640, width:'95vw', maxHeight:'92vh', display:'flex', flexDirection:'column' }}>
        <div className="modal-header" style={{ flexShrink:0 }}>
          <div className="modal-title">New PPAP Template</div>
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'20px 24px', display:'flex', flexDirection:'column', gap:14 }}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            <div className="form-group" style={{ margin:0 }}>
              ...
              <input className="form-input" value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Cummins PPAP" autoFocus />
            </div>
            <div className="form-group" style={{ margin:0 }}>
          ...
          <div>
            <div style={{ fontSize:11, fontWeight:600, color:'var(--text3)', textTransform:'uppercase', letterSpacing:'0.07em', marginBottom:10 }}>PPAP Elements</div>
            <div style={{ border:'1px solid var(--border)', borderRadius:8, overflow:'hidden' }}>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 100px 32px', background:'var(--bg2)', padding:'8px 12px', borderBottom:'1px solid var(--border)' }}>
...
function ViewPPAPTemplateModal({ templateId, onClose }: { templateId: string, onClose: () => void }) {
  const { data: template, isLoading } = useQuery({
    queryKey: ['ppap-template', templateId],
    ...
    <div className="modal-overlay open">
      <div className="modal" style={{ maxWidth:520, width:'95vw', maxHeight:'90vh', display:'flex', flexDirection:'column' }}>
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'20px 24px' }}>
          {isLoading ? <div style={{ textAlign:'center', padding:32, color:'var(--text4)' }}>Loading...</div> : (
            <>
