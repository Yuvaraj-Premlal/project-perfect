...
const { generatePreReviewBrief } = require('../services/ai');
...
async function generateWeeklyNarrative({ projectName, customerName, opv, lfv, vr, momentum, highRiskTasks, totalTasks, escalationActive, weekEnding }) {
  const { generateEscalationBrief } = require('../services/ai');
...
OPV (progress score): ${(opv * 100).toFixed(1)}% (target: above 80%)
LFV (load factor): ${(lfv * 100).toFixed(1)}%
VR (velocity ratio): ${(vr * 100).toFixed(1)}%
...
// List all weekly reports for a project
// ─────────────────────────────────────────────
